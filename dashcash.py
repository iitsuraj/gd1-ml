...
def predict(df3,value):
    X,y = df3[2].values.reshape(-1,1),df3[3].values.reshape(-1,1)
...
def run_data(): 
    ...
    if day=='0':
        ...
        conn = sqlite3.connect('user.db')
        conn.execute("INSERT INTO user%s VALUES (%s,%s,%s,%s)"%(user_id,day,day,day_n_1,str(day_n)));
        conn.commit()
        conn.close()
    else:
        day_n_1=df.values[-1][3]
